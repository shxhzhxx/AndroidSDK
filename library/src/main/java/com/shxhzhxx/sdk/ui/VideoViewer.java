...
public class VideoViewer extends FrameLayout implements TextureView.SurfaceTextureListener, View.OnClickListener, SeekBar.OnSeekBarChangeListener,
    ...
    private ConditionalAction mPlayCondRun = new ConditionalAction(3, new Function2<ConditionalAction,Integer, Unit>() {
        @Override
        public Unit invoke(ConditionalAction conditionalAction, Integer integer) {
            mPlayCondRun.set(2, false);
            mPreview.setVisibility(INVISIBLE);
            mPlayer.start();
            updateBtn();
            mUpdateProgressTask.run();
            mShowCtrlCondRun.set(1, true);
            return null;
        }
    });
    private ConditionalAction mShowCtrlCondRun = new ConditionalAction(2, new Function2<ConditionalAction,Integer, Unit>() {
        @Override
        public Unit invoke(ConditionalAction conditionalAction,Integer integer) {
    ...
    public void start() {
        if (mReleased)
            return;
        mPlayCondRun.set(2, true);
    }

    public void reset() {
        if (mReleased)
            return;
        mPlayCondRun.set(1, false);
        mShowCtrlCondRun.set(0, false);
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        mShowCtrlCondRun.set(1, true);
        mShowCtrlCondRun.set(0, true);
        mPlayCondRun.set(1, true);
    }
